...
# class_prefix_str = "KHYef"
# class_prefix_str = "IUEO"


nosearch_directorys = ["Pods","Model","fmdb","AppDelegate","AvoidCrash","WMPageController","LxlcoLIgmperhluen","Fuhxngjew","CQGJollyView","TIKDMindView","TIKDReapConclusionView","TIKDPollutionView","TIKDGravityStaticView"]
...
# 随机生成UIControlContentVerticalAlignment
def generate_random_ControlContentVerticalAlignment():
    random_index = random.randrange(0, 100)%4
    if random_index == 0:
        ...
        return "UIControlContentVerticalAlignmentFill"
# 随机生成UIControlContentHorizontalAlignment
def generate_random_ControlContentHorizontalAlignment():
    random_index = random.randrange(0, 100)%4
    if random_index == 0:
        ...
        return "UIControlContentHorizontalAlignmentFill"
# 随机生成UIEdgeInsetsSet
def generate_random_EdgeInsetsSet():
    random_index = random.randrange(0, 10)
    if random_index < 3:
        return "UIEdgeInsetsZero"    
    else:
        return "UIEdgeInsetsMake("+str(random.randrange(0, 25))+", "+str(random.randrange(0, 25))+", "+str(random.randrange(0, 25))+", "+str(random.randrange(0, 25))+")"
# 随机生成NSTextAlignment
def generate_random_textAlignment():
    random_index = random.randrange(0, 100)%5
    if random_index == 0:
        ...
        return "NSTextAlignmentCenter"
# 随机生成LineBreakMode
def generate_random_lineBreakMode():
    random_index = random.randrange(0, 100)%5
    if random_index == 0:
...
# 随机生成contenMode
def generate_random_contenMode():
    random_index = random.randrange(0, 100)%13
    if random_index == 0:
...
# 随机生成transofm-Rotate角度
def generate_transofm_Rotate(): 
    random_index = random.randrange(0, 100)%13
    if random_index == 0:
        return "M_E"
    elif random_index == 1:
        ...
        return "M_PI_4"
# 随机生成AttributedString字符串        
def generate_random_AttributedStringName():
    ...
    if random_index == 0:
        ...
        return "NSStrikethroughStyleAttributeName"#color
    elif random_index == 4:
...
def generate_random_Font():
    ...
    if random_index == 0:
        return "[UIFont systemFontOfSize:"+str(random.randrange(10, 20))+"]"
    elif random_index == 1:
        return "[UIFont boldSystemFontOfSize:"+str(random.randrange(10, 20))+"]"
    elif random_index == 2:
        return "[UIFont italicSystemFontOfSize:"+str(random.randrange(10, 20))+"]"
# 随机生成AttributedString字符串--number相关
def generate_random_Attributed_number():
    ...
    if random_index == 0:
        ...
        return "NSStrokeWidthAttributeName"
# 随机生成AttributedString字符串--文字书写方向相关
def generate_random_Attributed_deriction_array():
    random_index = random.randrange(0, 2)
    if random_index == 0:
        return "NSWritingDirectionAttributeName"
    elif random_index == 1:
        return "NSVerticalGlyphFormAttributeName"        
# 随机生成AttributedString字符串--color相关
def generate_random_Attributed_color():
    random_index = random.randrange(0, 5)
    if random_index == 0:
        return "NSForegroundColorAttributeName"
    elif random_index == 1:
        ...
        return "NSStrokeColorAttributeName"                
# 随机生成AttributedString字符串--double相关
def generate_random_Attributed_double():
    random_index = random.randrange(0, 4)
    if random_index == 0:
        return "NSUnderlineStyleAttributeName"
    elif random_index == 1:
...
# 输入框        
# 随机生成键盘类型
def generate_random_Keyboard_type():
    ...
    if random_index == 0:
        ...
        return "UIKeyboardTypeURL"#color
    elif random_index == 4:
...
# 随机生成键盘样式
def generate_random_Keyboard_color():
    random_index = random.randrange(0, 4)
    if random_index == 0:
...
# 随机生成return键样式
def generate_random_return_type():
...
# 随机生成输入框右边按钮清除样式
def generate_clear_leftview_mode():
    random_index = random.randrange(0, 4)
    if random_index == 0:
...
# 随机生成contentSize属性CGSizeMake字符串        
def generate_random_contentSizeMake():
    return "CGSizeMake("+str(random.uniform(0, 2))+", "+str(random.uniform(0, 2))+")"
    
# 随机生成offSet属性CGSizeMake字符串        
def generate_random_offSetSizeMake():
    return "CGSizeMake("+str(random.uniform(-2, 2))+", "+str(random.uniform(-2, 2))+")"

# 随机生成rgb颜色字符串
def generate_rgb_color():  
    return "[UIColor colorWithRed:"+str(random.randrange(0, 256))+"/255.0 green:"+str(random.randrange(0, 256))+"/255.0 blue:"+str(random.randrange(0, 256))+"/255.0 alpha:"+str(random.uniform(0.001, 0.03))+"]"
...
# 随机生成UIBlurEffect的style
def generate_blurEffect_style():
    random_index = random.randrange(0, 3)
    if random_index == 0:
...
# 自定义保留小数点后几位并四舍五入
def handle_transfer_defend_Garden6():
    out_str = ""
    random_var_Num1 = generate_random_str(out_random_type_method_name)
    random_var_Num2 = generate_random_str(out_random_type_method_name)

    random_var_str1 = generate_random_str(out_random_type_method_name)
    random_var_str2 = generate_random_str(out_random_type_method_name)

    random_var_NumToStr = generate_random_str(out_random_type_method_name)
    random_var_arr = generate_random_str(out_random_type_method_name)
    random_var_index = generate_random_str(out_random_type_method_name)

    random_decimalNum1 = generate_random_str(out_random_type_method_name)
    random_decimalNum2 = generate_random_str(out_random_type_method_name)
    random_decimalNum_New = generate_random_str(out_random_type_method_name)

    random_decimalNumHandler = generate_random_str(out_random_type_method_name)
    random_decimalNumResult = generate_random_str(out_random_type_method_name)


    # 数字1
    random_num = random.randrange(0,10)
    if random_num < 5:
        out_str += "\tNSNumber *"+random_var_Num1+" = [NSNumber numberWithDouble:"+str(random.randrange(0, 10000))+"."+str(random.randrange(0, 10000))+"];\n"
        out_str += "\tNSDecimalNumber* "+random_decimalNum1+" = [NSDecimalNumber decimalNumberWithString:["+random_var_Num1+" stringValue]];\n"
    else:
        out_str += "\tNSString *"+random_var_str1+" = @\""+str(random.randrange(0, 100))+"."+str(random.randrange(0, 1000))+"\";\n"
        out_str += "\tNSDecimalNumber* "+random_decimalNum1+" = [NSDecimalNumber decimalNumberWithString:"+random_var_str1+"];\n"

    # 数字2
    random_num1 = random.randrange(0,10)
    if random_num1 < 5:
        out_str += "\tNSNumber *"+random_var_Num2+" = [NSNumber numberWithDouble:"+str(random.randrange(0, 1000))+"."+str(random.randrange(0, 1000))+"];\n"
        out_str += "\tNSDecimalNumber* "+random_decimalNum2+" = [NSDecimalNumber decimalNumberWithString:["+random_var_Num2+" stringValue]];\n"
    else:
        out_str += "\tNSString *"+random_var_str2+" = @\""+str(random.randrange(0, 100))+"."+str(random.randrange(0, 1000))+"\";\n"
        out_str += "\tNSDecimalNumber* "+random_decimalNum2+" = [NSDecimalNumber decimalNumberWithString:"+random_var_str2+"];\n"

    # 比较大小

    out_str += "\tNSDecimalNumber *"+random_decimalNum_New+" = [[NSDecimalNumber alloc] init];\n"
    out_str += "\tif (["+random_decimalNum1+" compare:"+random_decimalNum2+"] == NSOrderedAscending) {\n"
    # random_decimalNum1 < random_decimalNum2
    random_num2 = random.randrange(0,10)
    if random_num2 < 5:
        # 加法
        out_str += "\t\t"+random_decimalNum_New+" = ["+random_decimalNum1+" decimalNumberByAdding:"+random_decimalNum2+"];\n"
    else:
        # 乘法
        out_str += "\t\t"+random_decimalNum_New+" = ["+random_decimalNum1+" decimalNumberByMultiplyingBy:"+random_decimalNum2+"];\n"


    out_str += "\t}else if(["+random_decimalNum1+" compare:"+random_decimalNum2+"] == NSOrderedDescending){\n"
    # random_decimalNum1 > random_decimalNum2
    random_num3 = random.randrange(0,10)
    if random_num3 < 5:
        # 减法
        out_str += "\t\t"+random_decimalNum_New+" = ["+random_decimalNum1+" decimalNumberBySubtracting:"+random_decimalNum2+"];\n"
    else:
        # 除法
        out_str += "\t\t"+random_decimalNum_New+" = ["+random_decimalNum1+" decimalNumberByDividingBy:"+random_decimalNum2+"];\n"

    out_str += "\t}else if (["+random_decimalNum1+" compare:"+random_decimalNum2+"] == NSOrderedSame){\n"
    # random_decimalNum1 = random_decimalNum2
    random_num4 = random.randrange(0,10)
    if random_num4 < 5:
        # 指数
        out_str += "\t\t"+random_decimalNum_New+" = ["+random_decimalNum1+" decimalNumberByMultiplyingByPowerOf10:"+str(random.randrange(0, 2))+"];\n"
    else:
        # 次方
        out_str += "\t\t"+random_decimalNum_New+" = ["+random_decimalNum2+" decimalNumberByRaisingToPower:"+str(random.randrange(0, 2))+"];\n"
  
    out_str += "\t}\n"

    # 将最总计算的结果转成字符串
    out_str += "\tNSString *"+random_var_NumToStr+" = ["+random_decimalNum_New+" stringValue];\n"
    out_str += "\tNSInteger "+random_var_index+";\n"
    out_str += "\tNSArray *"+random_var_arr+" = [NSArray array];\n"
    out_str += "\tif (["+random_var_NumToStr+" containsString:@\".\"]) {\n"
    out_str += "\t\t"+random_var_arr+" = ["+random_var_NumToStr+" componentsSeparatedByString:@\".\"];\n"
    out_str += "\t\t"+random_var_index+" = arc4random()%(["+random_var_arr+"[1] length]);\n"

    out_str += "\t}else{\n"
    out_str += "\t\t"+random_var_index+" = arc4random()%([["+random_var_arr+" lastObject] length]);\n"
    out_str += "\t}\n"

    # 四舍五入
    out_str += "\tNSDecimalNumberHandler *"+random_decimalNumHandler+" = [NSDecimalNumberHandler decimalNumberHandlerWithRoundingMode:NSRoundPlain scale:"+random_var_index+" raiseOnExactness:NO raiseOnOverflow:NO raiseOnUnderflow:NO raiseOnDivideByZero:NO];\n"

    out_str += "\t["+random_decimalNum_New+" decimalNumberByRoundingAccordingToBehavior:"+random_decimalNumHandler+"];\n"
...
# NSMutableDictionary
def handle_transfer_defend_Garden5():
    out_str = ""
    random_mut_dic = generate_random_str(out_random_type_method_name)
    random_dic = generate_random_str(out_random_type_method_name)    
    random_arr = generate_random_str(out_random_type_method_name)
    random_str = generate_random_str(out_random_type_method_name)
    random_mut_str = generate_random_str(out_random_type_method_name)
    random_long = generate_random_str(out_random_type_method_name)
    random_length = generate_random_str(out_random_type_method_name)
    random_index = generate_random_str(out_random_type_method_name)
    random_char = generate_random_str(out_random_type_method_name)
    random_range = generate_random_str(out_random_type_method_name)
    random_from_str = generate_random_str(out_random_type_method_name)
    random_to_str = generate_random_str(out_random_type_method_name)

    out_str += "NSMutableDictionary *"+random_mut_dic+" = [[NSMutableDictionary alloc] init];\n"
    out_str += "\tNSMutableString *"+random_mut_str+" = [[NSMutableString alloc] init];\n"
    random_num =random.randrange(0,10)%2
    if random_num == 0:
        out_str += "\tNSDictionary *"+random_dic+" = [[NSBundle mainBundle] infoDictionary];\n"

        random_num0 = random.randrange(0,10)%3
        if random_num0  == 0:
            # // app build版本
            out_str += "\tNSString *"+random_str+" = ["+random_dic+" objectForKey:@\"CFBundleVersion\"];\n"
        elif random_num0  == 1:
            # //Xcode 版本
            out_str += "\tNSString *"+random_str+" = ["+random_dic+" objectForKey:@\"DTXcode\"];\n"
        elif random_num0 == 2:
            # //SDK 的版本
            out_str += "\tNSString *"+random_str+" = ["+random_dic+" objectForKey:@\"DTSDKName\"];\n"

    elif random_num == 1:
        random_num1 = random.randrange(0,10)%5
        if random_num1 == 0:
            # //手机别名： 用户定义的名称
            out_str += "\tNSString *"+random_str+" = [[UIDevice currentDevice] name];\n"
        elif random_num1 == 1:
            # //设备名称
            out_str += "\tNSString *"+random_str+" = [[UIDevice currentDevice] systemName];\n"
        elif random_num1 == 2:
            # //手机系统版本
            out_str += "\tNSString *"+random_str+" = [[UIDevice currentDevice] systemVersion];\n"        
        elif random_num1 == 3:
            # //手机型号
            out_str += "\tNSString *"+random_str+" = [[UIDevice currentDevice] model];\n"   
        elif random_num1 == 4:
            # //地方型号  （国际化区域名称）
            out_str += "\tNSString *"+random_str+" = [[UIDevice currentDevice] localizedModel];\n"

    random_comp_str = generate_random_str(out_random_type_method_name)
    out_str += "\tNSString *"+random_comp_str+" = @\"" +generate_words_str("random_str")+ "\";\n"
    out_str += "\tlong "+random_long+" = ["+random_str+" compare:"+random_comp_str+"];\n"

    out_str += "\t"+random_str+" = [[NSNumber numberWithLong:"+random_long+"] stringValue];\n"
    out_str += "\t"+random_mut_str+" = ["+random_str+" mutableCopy];\n"
    random_num2 = random.randrange(0,10)
    if random_num2 < 3:
        out_str += "\t["+random_mut_str+" insertString:@\"" +generate_words_str("random_str")+ "\" atIndex:arc4random()%"+random_str+".length];\n"
    else:
        out_str += "\t["+random_mut_str+" setString:"+random_comp_str+"];\n"
    random_num3 = random.randrange(0,10)
    if random_num3 < 5:
        if random_num3%2 == 0:
            out_str += "\tNSString *"+random_from_str+" = ["+random_str+" substringFromIndex:arc4random()%"+random_str+".length];\n"
            out_str += "\t["+random_mut_dic+" setValue:"+random_from_str+" forKey:@\"" +generate_words_str("random_str")+ "\"];\n"
        elif random_num3%2 ==1:
            out_str += "\tNSString *"+random_to_str+" = ["+random_str+" substringToIndex:arc4random()%"+random_str+".length];\n"
            out_str += "\t["+random_mut_dic+" setObject:"+random_to_str+" forKey:@\"" +generate_words_str("random_str")+ "\"];\n"
    else:
        out_str += "\tNSArray *"+random_arr+"= [NSArray array];\n"
        out_str += "\t"+random_arr+" = ["+random_str+" componentsSeparatedByString:"+random_comp_str+"];\n"
        out_str += "\t"+random_arr+" = [["+random_arr+" reverseObjectEnumerator] allObjects];\n"
    out_str += "\t["+random_mut_str+" appendString:@\"" +generate_words_str("random_str")+ "\"];\n"
    random_num4 = random.randrange(0,10)
    if random_num4 < 5:
        out_str += "\tNSRange "+random_range+" = {"+str(random.randrange(0, 3))+","+str(random.randrange(3, 5))+"};\n"
        out_str += "\t["+random_mut_str+" replaceCharactersInRange:"+random_range+" withString:@\"" +generate_words_str("random_str")+ "\"];\n"
    else:
        out_str += "\tNSRange "+random_range+" = {"+str(random.randrange(0, 3))+","+str(random.randrange(3, 5))+"};\n"
        out_str += "\t"+random_str+" = [["+random_mut_str+" substringWithRange:"+random_range+"] mutableCopy];\n"
    random_num0 = random.randrange(0,10)
    if random_num0 < 5:
        out_str += "\tconst char *"+random_char+" = NULL;\n"
        out_str += "\tif (["+random_str+" canBeConvertedToEncoding:NSUTF8StringEncoding]) {\n"
        out_str += "\t\t"+random_char+" = ["+random_str+" cStringUsingEncoding:NSUTF8StringEncoding];\n"
        out_str += "\t}\n"
    else:
        # out_str += "\tconst char *"+random_char+" = NULL;\n"        
        out_str += "\tNSUInteger "+random_index+" = arc4random()%"+random_str+".length;\n"
        out_str += "\t["+random_str+" characterAtIndex:"+random_index+"];\n"
        # out_str += "\t"+random_str+" = [NSString stringWithCString:"+random_char+"encoding:NSUTF8StringEncoding];\n"
    out_str += "\t["+random_mut_dic+" removeAllObjects];\n"
    return out_str

# NSMutableArray
def handle_transfer_defend_Garden4():
    out_str = ""
    random_str = generate_random_str(out_random_type_method_init)
    random_mut_str = generate_random_str(out_random_type_method_init)
    random_mut_arr = generate_random_str(out_random_type_method_init)
    random_mut_data = generate_random_str(out_random_type_method_init)

    random_insert_str = generate_random_str(out_random_type_method_init)
    random_index = generate_random_str(out_random_type_method_init)
    random_arr_index = generate_random_str(out_random_type_method_init)

    random_sub_str1 = generate_random_str(out_random_type_method_init)
    random_sub_str2 = generate_random_str(out_random_type_method_init)

    out_str += "NSMutableArray *"+random_mut_arr+" = [NSMutableArray array];\n"

    out_str += "\tNSString *"+random_str+" = @\"" +generate_words_str("random_str")+ "\";\n"
    out_str += "\tNSInteger "+random_index+" = "+random_str+".length/2;\n"
    out_str += "\tNSMutableString *" + random_mut_str + " = ["+random_str+" mutableCopy];\n"

    out_str += "\tNSString *"+random_insert_str+" = @\"" +generate_words_str("random_str")+ "\";\n"

    out_str += "\t["+random_mut_str+" insertString:"+random_insert_str+" atIndex:"+random_index+"];\n"

    random_num1 = random.randrange(0,10)
    if random_num1 < 5:
        out_str += "\tNSString *"+random_sub_str1+" = ["+random_mut_str+" substringToIndex:"+random_index+"];\n"
        out_str += "\tNSString *"+random_sub_str2+" = ["+random_mut_str+" substringFromIndex:"+random_index+"];\n"

    else:
        out_str += "\tNSString *"+random_sub_str1+" = ["+random_mut_str+" substringWithRange:NSMakeRange(0, "+random_index+")];\n"
        out_str += "\tNSString *"+random_sub_str2+" = ["+random_mut_str+" substringWithRange:NSMakeRange("+random_index+", "+random_mut_str+".length-"+random_index+")];\n"

    random_num2 = random.randrange(0,10)%3
    if random_num2 == 0:
        out_str += "\t["+random_mut_arr+" addObject:"+random_sub_str1+"];\n"
        out_str += "\t["+random_mut_arr+" addObject:"+random_sub_str2+"];\n"        
    elif random_num2 == 1:
        out_str += "\t["+random_mut_arr+" addObjectsFromArray:@["+random_sub_str1+","+random_sub_str2+"]];\n"
    elif random_num2 == 2:
        out_str += "\t["+random_mut_arr+" setArray:@["+random_sub_str1+","+random_sub_str2+"]];\n"
    # random_num2_2 = random.randrange(0,10)
    # if random_num2_2 < 5:
    #     out_str += "\t["+random_mut_arr+" enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {\n"
    #     out_str += "\t\tif ([obj isKindOfClass:[NSIndexSet class]]) {\n"
    #     out_str += "\t\t\t["+random_mut_arr+" removeObject:obj];\n"
    #     # out_str += "\t\t\t"+random_mut_arr+"[idx] = [[obj string] stringByReplacingOccurrencesOfString:"+random_insert_str+" withString:@\"\"];\n"
    #     out_str += "\t\t}\n"
    #     out_str += "\t}];\n"
    # else:
    #     out_str += "\tfor (int i = 0; i < "+random_mut_arr+".count; i++) {\n"
    #     out_str += "\t\tif (["+random_mut_arr+"[i] isKindOfClass:[NSNumber class]]) {\n"
    #     out_str += "\t\t\t["+random_mut_arr+" removeObject:"+random_mut_arr+"[i]];\n"
    #     out_str += "\t\t}\n"
    #     out_str += "\t}\n"

        # out_str += "\t"+random_mut_arr+" = [["+random_mut_str+" componentsSeparatedByString:"+random_insert_str+"] mutableCopy];\n"

    random_num2_2 = random.randrange(0,10)
    if random_num2_2 < 5:
        out_str += "\tif ("+random_mut_arr+".count <= 1) {\n"
        out_str += "\t\t["+random_mut_arr+" insertObject:@\"" +generate_words_str("random_str")+ "\" atIndex:0];\n"
        out_str += "\t}\n"

        out_str += "\telse{\n"
        out_str += "\t\t["+random_mut_arr+" removeLastObject];\n"
        out_str += "\t\t["+random_mut_arr+" addObject:@\"" +generate_words_str("random_str")+ "\"];\n"
        out_str += "\t}\n"
    else:
        out_str += "\t["+random_mut_arr+" setArray:@[@\"" +generate_words_str("random_str")+ "\",@\"" +generate_words_str("random_str")+ "\"]];\n"

    random_num3 = random.randrange(0,10)%2
    if random_num3 == 0:
        out_str += "\t["+random_mut_arr+" exchangeObjectAtIndex:0 withObjectAtIndex:1];\n"
    elif random_num3 == 1:
        out_str += "\t"+random_mut_arr+" = [[["+random_mut_arr+" reverseObjectEnumerator] allObjects] mutableCopy];\n"

    random_var2 = generate_random_str(out_random_type_method_init)
    out_str += "\tCGFloat " + random_var2 + " = " + str(random.randrange(0, 100)) + "." + str(random.randrange(0, 1000)) + ";\n"
    random_var3 = generate_random_str(out_random_type_method_init)
    out_str += "\tNSString *" + random_var3 + " = [NSString stringWithFormat:@\"%f\", " + random_var2 + "];\n"

    random_num4 = random.randrange(0,10)
    if random_num4 < 3:
        out_str += "\t["+random_mut_arr+" insertObject:"+random_var3+" atIndex:arc4random()%"+random_mut_arr+".count];\n"

    random_num5 = random.randrange(0,10)
    if random_num5 < 5:
        out_str += "\t["+random_mut_arr+" replaceObjectAtIndex:arc4random()%"+random_mut_arr+".count withObject:@\"" +generate_words_str("random_str")+ "\"];\n"
    
    out_str += "\tif (["+random_mut_arr+" containsObject:"+random_var3+"]) {\n"
    out_str += "\t\t["+random_mut_arr+" removeObject:"+random_var3+"];\n"
    out_str += "\t}else{\n"
    out_str += "\t\t["+random_mut_arr+" addObject:"+random_var3+"];\n"
    out_str += "\t}\n"

    random_num6 = random.randrange(0,10)
    if random_num6 < 5:
        random_arr_index = "arc4random()%"+random_mut_arr+".count"
        out_str += "\t"+random_mut_arr+"["+random_arr_index+"] = ["+random_mut_arr+"["+random_arr_index+"] stringByAppendingString:@\"" +generate_words_str("random_str")+ "\"];\n"

        # out_str += "\t["+random_mut_arr+" enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {\n"
        # out_str += "\t\t"+random_mut_arr+"[idx] = ["+random_mut_arr+"[idx] stringByAppendingString:@\"" +generate_words_str("random_str")+ "\"];\n"
        # out_str += "\t}];\n"

    random_num7 = random.randrange(0,10)
    if random_num7 < 5:
        out_str += "\tNSMutableData *" + random_mut_data + " = [[NSMutableData alloc] initWithBase64EncodedString:"+random_mut_arr+"[arc4random()%"+random_mut_arr+".count] options:NSDataBase64DecodingIgnoreUnknownCharacters];\n"
    else:
        out_str += "\tNSMutableData *" + random_mut_data + " = [[NSMutableData alloc] initWithBase64EncodedString:@\"" + generate_words_str("random_str") + "\" options:NSDataBase64DecodingIgnoreUnknownCharacters];\n"
    
    random_num8 = random.randrange(0,10)
    if random_num8 < 3: 
        out_str += "\t["+random_mut_data+" resetBytesInRange:NSMakeRange(0, arc4random()%"+random_mut_data+".length)];\n"
    else:
        out_str += "\t["+random_mut_data+" setLength:0];\n"
...
# NSDate
def handle_transfer_defend_Garden3():
    out_str = ""
    random_str = generate_random_str(out_random_type_method_name)
    random_Date = generate_random_str(out_random_type_method_name)
    random_GMT_Date = generate_random_str(out_random_type_method_name)   

    random_Date_Formatter = generate_random_str(out_random_type_method_name) 
    random_Time_Interval = generate_random_str(out_random_type_method_name)
    random_Day_Interval = generate_random_str(out_random_type_method_name)

    random_mut_str = generate_random_str(out_random_type_method_name)
    random_arr = generate_random_str(out_random_type_method_name)
    random_mut_arr = generate_random_str(out_random_type_method_name)
    random_mut_dic = generate_random_str(out_random_type_method_name)

    random_range = generate_random_str(out_random_type_method_name)
    random_mut_attribute_str = generate_random_str(out_random_type_method_name)

    out_str += "NSString *"+random_str+" = [NSString string];\n"
    random_num = random.randrange(0,10)%3
    if random_num == 0:
        # 现在的时间
        out_str += "\tNSDate *"+random_Date+" = [NSDate date];\n"

    elif random_num == 1:
        # 未来的一个时间
        out_str += "\tNSDate *"+random_Date+" = [NSDate distantFuture];\n"

    elif random_num == 2:
        # 远古的一个时间
        out_str += "\tNSDate *"+random_Date+" = [NSDate distantPast];\n"

    out_str += "\tNSMutableArray *"+random_mut_arr+" = [NSMutableArray array];\n"

    out_str += "\t["+random_mut_arr+" setArray:@["+random_Date+"]];\n"

    out_str += "\tNSMutableDictionary *"+random_mut_dic+" = [NSMutableDictionary dictionary];\n"

    out_str += "\tNSDateFormatter *"+random_Date_Formatter+" = [[NSDateFormatter alloc] init];\n"
    out_str += "\t["+random_Date_Formatter+" setDateFormat:@\"yyyy/MM/dd HH:mm:ss\"];\n"

    out_str += "\tNSTimeInterval "+random_Time_Interval+" = [[NSTimeZone systemTimeZone] secondsFromGMT];\n"
    out_str += "\tNSDate *"+random_GMT_Date+" = ["+random_Date+" dateByAddingTimeInterval:"+random_Time_Interval+"];\n"

    random_num1 = random.randrange(0,10)
    if random_num1 < 3:
        out_str += "\tNSTimeInterval "+random_Day_Interval+" = 24 * 60 * 60;\n"
        random_num1_1 = random.randrange(0,10)%3
        if random_num1_1 == 0:
            # 昨天
            out_str += "\t["+random_mut_arr+" addObject:["+random_GMT_Date+" dateByAddingTimeInterval:-"+random_Day_Interval+"]];\n"
        elif random_num1_1 == 1:
            # 今天
            out_str += "\t["+random_mut_arr+" addObject:["+random_GMT_Date+" dateByAddingTimeInterval:0]];\n"
        elif random_num1_1 == 2:
            # 明天
            out_str += "\t["+random_mut_arr+" addObject:["+random_GMT_Date+" dateByAddingTimeInterval:"+random_Day_Interval+"]];\n"

    else:
        out_str += "\tif (["+random_Date+" isEqualToDate:"+random_GMT_Date+"]) {\n"
        # 将时间戳转换成字符串
        out_str += "\t\t"+random_str+" = ["+random_Date_Formatter+" stringFromDate:"+random_Date+"];\n"
        out_str += "\t\t["+random_mut_dic+" setObject:"+random_str+" forKey:@\"" +generate_words_str("random_str")+ "\"];\n"
        out_str += "\t} else {\n"
        out_str += "\t\t"+random_str+" = ["+random_Date_Formatter+" stringFromDate:"+random_GMT_Date+"];\n"
        out_str += "\t\t["+random_mut_dic+" setObject:"+random_str+" forKey:@\"" +generate_words_str("random_str")+ "\"];\n"
        out_str += "\t}\n"
    ...
    if random_num2 < 5:
        ...
        # 设置时区时间
        out_str += "\t["+random_Date_Formatter+" setTimeZone:[NSTimeZone timeZoneForSecondsFromGMT:0]];\n"
        # 将时间字符串转换成日期
        out_str += "\t"+random_Date+" = ["+random_Date_Formatter+" dateFromString:"+random_str+"];\n"
        out_str += "\t["+random_mut_arr+" arrayByAddingObject:"+random_Date+"];\n"
    else:  
        out_str += "\t"+random_str+" = @\"" +str(random.randrange(34500000,34567898))+ "\";\n"

        out_str += "\t"+random_Date+" = [NSDate dateWithTimeIntervalSince1970:["+random_str+" doubleValue]];\n"
        out_str += "\t["+random_mut_arr+" arrayByAddingObject:"+random_Date+"];\n"

    random_num3 = random.randrange(0,10)
    if random_num3 < 5:
        out_str += "\t["+random_mut_arr+" removeObjectAtIndex:arc4random()%"+random_mut_arr+".count];\n"
    else:
        out_str += "\t["+random_mut_arr+" insertObject:@\"" +generate_words_str("random_str")+ "\" atIndex:arc4random()%"+random_mut_arr+".count];\n"

    random_num4 = random.randrange(0,10)
    if random_num4 < 2:

        # 将时间戳转换成字符串
        out_str += "\tif ("+random_mut_arr+".count <= "+str(random.randrange(1,4))+") {\n"
        out_str += "\t\t["+random_mut_arr+" enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {\n"
        out_str += "\t\t\tif ([obj isKindOfClass:[NSString class]]) {\n"
        out_str += "\t\t\t\tNSMutableAttributedString *"+random_mut_attribute_str+" = [[NSMutableAttributedString alloc] initWithString:obj];\n"

        random_num4_4 = random.randrange(0,10)%4
        if random_num4_4 == 0:
            out_str += "\t\t\t\t["+random_mut_attribute_str+" addAttribute:NSFontAttributeName value:"+generate_random_Font()+" range:NSMakeRange(0, "+random_mut_attribute_str+".length)];\n"
        elif random_num4_4 == 1:
            out_str += "\t\t\t\t["+random_mut_attribute_str+" addAttribute:"+generate_random_Attributed_color()+" value:"+generate_rgb_color()+" range:NSMakeRange(0, "+random_mut_attribute_str+".length)];\n"
        elif random_num4_4 == 2:
            out_str += "\t\t\t\t["+random_mut_attribute_str+" addAttribute:"+generate_random_Attributed_double()+" value:@("+str(random.randrange(1,10))+"/10.0) range:NSMakeRange(0, "+random_mut_attribute_str+".length)];\n"
        elif random_num4_4 == 3:
            out_str += "\t\t\t\t["+random_mut_attribute_str+" addAttribute:"+generate_random_Attributed_number()+" value:@"+str(random.randrange(1,10))+" range:NSMakeRange(0, "+random_mut_attribute_str+".length)];\n"
        ...
        random_num5 = random.randrange(0,10)
        if random_num5 < 5:
          out_str += "\t["+random_mut_arr+" removeAllObjects];\n"
        else:
          out_str += "\t["+random_mut_arr+" addObject:@\"" +generate_words_str("random_str")+ "\"];\n"
    random_num6 = random.randrange(0,10)
    if random_num6 < 2:
    	random_num8 = random.randrange(0,10)
    	if random_num8 < 5:
	        out_str += "\t["+random_mut_dic+" setValue:@\"" +generate_words_str("random_str")+ "\" forKey:@\"" +generate_words_str("random_str")+ "\"];\n"
        # else:
	       #  out_str += "\t["+random_mut_dic+" enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {\n"
	       #  out_str += "\t\tif ([key isEqualToString:@\"" +generate_words_str("random_str")+ "\"]) {\n"
	       #  out_str += "\t\t\t["+random_mut_dic+" removeObjectForKey:key];\n"
	       #  out_str += "\t\t}\n"
	       #  out_str += "\t}];\n"
    else:  
        random_num9 = random.randrange(0,10)
        if random_num9 < 5:
            out_str += "\t["+random_mut_dic+" removeObjectForKey:@\"" +generate_words_str("random_str")+ "\"];\n"
        else:
            out_str += "\t["+random_mut_dic+" removeAllObjects];\n"
...
def handle_transfer_defend_Garden2():
    out_str = ""
    randomNum = generate_random_str(out_random_type_method_name)
    random_mut_str = generate_random_str(out_random_type_method_name)
    random_arr = generate_random_str(out_random_type_method_name)
    random_mut_arr = generate_random_str(out_random_type_method_name)
    random_str = generate_random_str(out_random_type_method_name)
    random_int = generate_random_str(out_random_type_method_name)
    random_behavior = generate_random_str(out_random_type_method_name)
    random_DecimalNumber = generate_random_str(out_random_type_method_name)
    random_mut_attribute_str = generate_random_str(out_random_type_method_name)
    random_Predicate = generate_random_str(out_random_type_method_name)
    random_CharacterSet = generate_random_str(out_random_type_method_name)
    random_CharacterSet2 = generate_random_str(out_random_type_method_name)
    random_Scanner = generate_random_str(out_random_type_method_name)
    random_index = generate_random_str(out_random_type_method_name)
    random_header = generate_random_str(out_random_type_method_name)
    random_footer = generate_random_str(out_random_type_method_name)

    random_Range = generate_random_str(out_random_type_method_name)
    random_Point = generate_random_str(out_random_type_method_name)
    random_Size = generate_random_str(out_random_type_method_name)
    random_Rect = generate_random_str(out_random_type_method_name)


    out_str += "NSNumber *"+randomNum+" = [NSNumber new];\n"
    out_str += "\tNSArray *"+random_arr+" = [NSArray array];\n"

    random_num = random.randrange(0,10)%2
    if random_num == 0:
        out_str += "\t"+randomNum+" = @"+str(random.randrange(0,1000000))+";\n"
    elif random_num == 1:
        out_str += "\t"+randomNum+" = @"+str(random.randrange(0,1000))+"."+str(random.randrange(10,1000000))+";\n"

    random_num0 = random.randrange(0,10)
    if random_num0 < 3:
        out_str += "\tNSMutableString *"+random_mut_str+" = [[NSMutableString alloc] initWithFormat:@\"%@\",["+randomNum+" stringValue]];\n"
    else:
        out_str += "\tNSMutableString *"+random_mut_str+" = [[NSMutableString alloc] initWithString:@\"" +generate_words_str("random_str")+ "\"];\n"
    # 判断是浮点数
    out_str += "\tif (["+random_mut_str+" containsString:@\".\"]) {\n"
    # 中间内容
    random_num0_0 = random.randrange(0,10)
    if random_num0_0 < 3:
        out_str += "\t\t"+random_arr+" = ["+random_mut_str+" componentsSeparatedByString:@\".\"];\n"
        out_str += "\t\t"+random_mut_str+" = [["+random_arr+" componentsJoinedByString:@\"\"] mutableCopy];\n"
    else:
        # 四舍五入保留2位小数
        out_str += "\t\tNSDecimalNumberHandler *"+random_behavior+" = [NSDecimalNumberHandler decimalNumberHandlerWithRoundingMode:NSRoundPlain scale:"+str(random.randrange(1,4))+" raiseOnExactness:NO raiseOnOverflow:NO raiseOnUnderflow:NO raiseOnDivideByZero:NO];\n"
        out_str += "\t\tNSDecimalNumber *"+random_DecimalNumber+" = [NSDecimalNumber decimalNumberWithString:"+random_mut_str+"];\n"
        out_str += "\t\t"+random_mut_str+" = [[["+random_DecimalNumber+" decimalNumberByRoundingAccordingToBehavior:"+random_behavior+"] stringValue] mutableCopy];\n"
        out_str += "\t\t"+random_arr+" = ["+random_mut_str+" componentsSeparatedByString:@\".\"];\n"
    ...
    out_str += "\telse{\n"
    # 必要内容
    out_str += "\t\tNSInteger "+random_index+" = "+random_mut_str+".length/2;\n"
    out_str += "\t\tNSString *"+random_header+" = ["+random_mut_str+" substringToIndex:"+random_index+"];\n"
    out_str += "\t\tNSString *"+random_footer+" = ["+random_mut_str+" substringFromIndex:"+random_index+"];\n"
    out_str += "\t\t"+random_arr+" = @["+random_header+","+random_footer+"];\n"
    # 中间内容
    random_num0__0 = random.randrange(0,10)%4
    if random_num0__0 == 0:
        out_str += "\t\tNSString *"+random_str+" = @\"[0-9]*\";\n"
        out_str += "\t\tNSPredicate *"+random_Predicate+" = [NSPredicate predicateWithFormat:@\"SELF MATCHES %@\","+random_str+"];\n"
        out_str += "\t\tif (["+random_Predicate+" evaluateWithObject:"+random_mut_str+"]) {\n"

        out_str += "\t\t\tNSRange "+random_Range+";\n"
        out_str += "\t\t\t"+random_Range+" = NSMakeRange(["+random_arr+"[0] floatValue], ["+random_arr+"[1] floatValue]);\n"

        out_str += "\t\t}\n"
    elif random_num0__0 == 1:
        out_str += "\t\tNSCharacterSet *"+random_CharacterSet+" = [NSCharacterSet decimalDigitCharacterSet];\n"
        out_str += "\t\t"+random_mut_str+" = [["+random_mut_str+" stringByTrimmingCharactersInSet:"+random_CharacterSet+"] mutableCopy];\n"
        out_str += "\t\tif ("+random_mut_str+".length < 0) {\n"

        out_str += "\t\t\tCGPoint "+random_Point+";\n"
        out_str += "\t\t\t"+random_Point+" = CGPointMake(["+random_arr+"[0] floatValue], ["+random_arr+"[1] floatValue]);\n"

        out_str += "\t\t}\n"
    elif random_num0__0 == 2:
        out_str += "\t\tNSCharacterSet *"+random_CharacterSet2+" = [[NSCharacterSet characterSetWithCharactersInString:@\".1234567890-\"] invertedSet];\n"
        out_str += "\t\tNSString *"+random_str+" = [["+random_mut_str+" componentsSeparatedByCharactersInSet:"+random_CharacterSet2+"] componentsJoinedByString:@\"\"];\n"
        out_str += "\t\tif (["+random_mut_str+" isEqualToString:"+random_str+"]) {\n"

        out_str += "\t\t\tCGSize "+random_Size+";\n"
        out_str += "\t\t\t"+random_Size+" = CGSizeMake(["+random_arr+"[0] floatValue], ["+random_arr+"[1] floatValue]);\n"

        out_str += "\t\t}\n"
    elif random_num0__0 == 3:
        out_str += "\t\tNSScanner *"+random_Scanner+" = [NSScanner scannerWithString:"+random_mut_str+"];\n"
        out_str += "\t\tint "+random_int+";\n"

        out_str += "\t\tif (["+random_Scanner+" scanInt:&"+random_int+"] && ["+random_Scanner+" isAtEnd]) {\n"

        out_str += "\t\t\tCGRect "+random_Rect+";\n"
        out_str += "\t\t\t"+random_Rect+" = CGRectMake("+str(random.randrange(0,100))+", "+str(random.randrange(0,100))+", ["+random_arr+"[0] floatValue], ["+random_arr+"[1] floatValue]);\n"

        out_str += "\t\t}\n"
    out_str += "\t}\n"


    # random_num1 = randrange(0,10)
    # if random_num1 < 5:    

    # random_num2 = randrange(0,10)
    # if random_num2 < 3:    

    # random_num3 = randrange(0,10)
    # if random_num3 < 5:    

    # random_num4 = randrange(0,10)
    # if random_num4 < 3:    

    # random_num5 = randrange(0,10)
    # if random_num5 < 5:    

    # random_num6 = randrange(0,10)
    # if random_num6 < 3:    

    # random_num7 = randrange(0,10)
    # if random_num7 < 5:    
...
def handle_transfer_defend_Garden():
    ...
    # random_new_str = generate_random_str(out_random_type_method_name)
    random_number = generate_random_str(out_random_type_method_name)
    random_substr1 = generate_random_str(out_random_type_method_name)
    random_substr2 = generate_random_str(out_random_type_method_name)
    # random_var1 = generate_random_str(out_random_type_method_name)
    # random_var2 = generate_random_str(out_random_type_method_name)
    # random_var3 = generate_random_str(out_random_type_method_name)

    random_num = random.randrange(0,10)%5
    if random_num == 0:
        # //手机别名： 用户定义的名称
        out_str += "NSString *"+random_str+" = [[UIDevice currentDevice] name];\n"
    elif random_num == 1:
    ...
    out_str += "\tNSString *" + random_substr1 + " = [" + random_str + " substringFromIndex:" + random_str + ".length/2];\n"
    out_str += "\tNSString *" + random_substr2 + " = [" + random_str + " substringToIndex:" + random_str + ".length/2];\n"
    out_str += "\tNSArray *" + random_arr + " = @["+random_substr1+","+random_substr2+"];\n"
    random_num1 = random.randrange(0,10)
    if random_num1 < 5:
        out_str += "\tNSMutableArray *" + random_mut_arr + " = [[NSMutableArray alloc] init];\n"
        out_str += "\t"+random_mut_arr+" = ["+random_arr+" mutableCopy];\n"
    else:
        out_str += "\tNSMutableArray *" + random_mut_arr + " = [[NSMutableArray alloc] initWithArray:"+random_arr+"];\n"

    for i in range(random.randrange(1, 3)):
        out_str += "\t[" + random_mut_arr + " insertObject:@\"" + generate_words_str("random_str") + "\" atIndex:arc4random()%"+random_mut_arr+".count];\n"
        out_str += "\t[" + random_mut_arr + " addObject:@\"" + generate_words_str("random_str") + "\"];\n"

    out_str += "\tNSMutableString *"+random_mut_str+" = [NSMutableString string];\n"        
    out_str += "\t"+random_mut_str+" = [["+random_mut_arr+" componentsJoinedByString:@\"\"] mutableCopy];\n"
    random_num2 = random.randrange(0,10)%2
    if random_num2 == 0:
        random_num2_2 = random.randrange(0,10)%3
        if random_num2_2 == 0:
            out_str += "\tNSNumber *"+random_number+" = [NSNumber numberWithInt:"+str(random.randrange(0, 1000))+"];\n"        
        elif random_num2_2 == 1:
            out_str += "\tNSNumber *"+random_number+" = [NSNumber numberWithLongLong:"+str(random.randrange(0, 1000))+"];\n"        
        elif random_num2_2 == 2:
            out_str += "\tNSNumber *"+random_number+" = [NSNumber numberWithDouble:"+str(random.randrange(0, 100))+"."+str(random.randrange(0, 100))+"];\n"
        out_str += "\t["+random_mut_str+" appendFormat:@\"%@\","+random_number+"];\n"
    else:
        out_str += "\t["+random_mut_str+" appendString:@\"" + generate_words_str("random_str") + "\"];\n"

    out_str += "\tNSRange "+random_range+" = {"+str(random.randrange(0, 7))+","+str(random.randrange(7, 12))+"};\n"        
    random_num3 = random.randrange(0,10)
    if random_num3 < 5:
        out_str += "\t["+random_mut_str+" deleteCharactersInRange:"+random_range+"];\n"
    else:
        out_str += "\t["+random_mut_str+" replaceCharactersInRange:"+random_range+" withString:@\"" + generate_words_str("random_str") + "\"];\n"

    random_num4 = random.randrange(0,10)
    if random_num4 < 3:
        out_str += "\tNSMutableAttributedString *" + random_mut_attribute_str + " = [[NSMutableAttributedString alloc] initWithString:"+random_mut_str+"];\n"
        random_num1 = random.randrange(0, 10)
        if random_num1 < 5:         
            out_str += "\t[" + random_mut_attribute_str + " addAttribute:"+generate_random_Attributed_color()+" value:"+generate_rgb_color()+" range:NSMakeRange(0, "+random_mut_attribute_str+".length)];\n"
        random_num2 = random.randrange(0, 10)
        if random_num2 < 3:         
            out_str += "\t[" + random_mut_attribute_str + " addAttribute:"+generate_random_Attributed_double()+" value:@0."+str(random.randrange(1,10))+" range:NSMakeRange(0, "+random_mut_attribute_str+".length)];\n"
        random_num3 = random.randrange(0, 10)
        if random_num3 < 5:         
            out_str += "\t[" + random_mut_attribute_str + " addAttribute:NSFontAttributeName value:"+generate_0905_param_str("UIFont *")+" range:NSMakeRange(0, "+random_mut_attribute_str+".length)];\n"
            # out_str += "\t[" + random_mut_attribute_str + " addAttribute:NSFontAttributeName value:"+generate_0905_param_str("UIFont *")+" range:NSMakeRange(0, "+random_mut_attribute_str+".length)];\n"

        random_num4 = random.randrange(0, 10)
        if random_num4 < 5:        
            out_str += "\t[" + random_mut_attribute_str + " addAttribute:"+generate_random_Attributed_number()+" value:@"+str(random.randrange(2,10))+" range:NSMakeRange(0, "+random_mut_attribute_str+".length)];\n"

        random_paragraph_style = generate_random_str(out_random_type_class)
        out_str += "\tNSMutableParagraphStyle *" + random_paragraph_style + " = [[NSMutableParagraphStyle alloc] init];\n"        
        out_str += "\t" + random_paragraph_style + ".firstLineHeadIndent = "+str(random.randrange(10,20))+";\n"
        random_num5 = random.randrange(0, 10)
        if random_num5 < 3:         
            out_str += "\t" + random_paragraph_style + ".lineBreakMode = "+generate_random_lineBreakMode()+";\n"
        out_str += "\t" + random_paragraph_style + ".lineSpacing = "+str(random.randrange(2,10))+";\n"
        random_num6 = random.randrange(0, 10)
        if random_num6 < 3:   
            out_str += "\t" + random_paragraph_style + ".lineHeightMultiple = "+str(random.randrange(2,10))+";\n"
        out_str += "\t[" + random_mut_attribute_str + " addAttribute:NSParagraphStyleAttributeName value:"+random_paragraph_style+" range:NSMakeRange(0, "+random_mut_attribute_str+".length)];\n"
        
        random_num7 = random.randrange(0, 10)
        if random_num7 < 3:
            random_shadow = generate_random_str(out_random_type_class)
            out_str += "\tNSShadow *" + random_shadow + " = [[NSShadow alloc] init];\n"        
            out_str += "\t" + random_shadow + ".shadowColor = "+generate_0905_param_str("UIColor *")+";\n"
            out_str += "\t" + random_shadow + ".shadowOffset = "+generate_random_offSetSizeMake()+";\n"
            out_str += "\t[" + random_mut_attribute_str + " addAttribute:NSShadowAttributeName value:"+random_shadow+" range:NSMakeRange(0, "+random_mut_attribute_str+".length)];\n"
        ...
        if len(file_context) > 0:
            ...
            for line in file_context:
                if "//////insert//" in line:
                    ...
                    # need_replace_str += generate_handle_string_data(random_var)
                    random_num = random.randrange(0,10)%6
                    if random_num == 0:
                        need_replace_str += handle_transfer_defend_Garden1()                        
                    elif random_num == 1:
                        need_replace_str += handle_transfer_defend_Garden2()
                    elif random_num == 2:
                        need_replace_str += handle_transfer_defend_Garden3()                        
                    elif random_num == 3:
                        need_replace_str += handle_transfer_defend_Garden4()    
                    elif random_num == 4:
                        need_replace_str += handle_transfer_defend_Garden5()  
                    elif random_num == 5:
                        need_replace_str += handle_transfer_defend_Garden6()  
...
def contains_addSubview_1008(target_str):
    ...
    if " addSubview:" in target_str:
        if "[UIApplication sharedApplication].keyWindow addSubview:" in target_str:
            contains = False
        else:
            contains = True
    # elif "[UIApplication sharedApplication].keyWindow addSubview:" in target_str:
    #     contains = False
    return contains
...
def get_insert_0919_handle_class_obj(use_insert_class_list, insert_0905_type):
    ...
    class_name = "IUEOuweKUoeijdIweokldldLo"
    for insert_class_obj in use_insert_class_list:
        if class_name in insert_class_obj.class_name:
            return insert_class_obj
...
def get_insert_0912_handle_class_obj(use_insert_class_list, insert_0905_type):
    class_name = ""
    if insert_0905_type == global_0905insert_type_longlong:
        class_name = "Tgjkd0912DFJkHandleLo"
    elif insert_0905_type == global_0905insert_type_NSString:
        class_name = "Tdjfko0912hfdkjHandleSt"
    elif insert_0905_type == global_0905insert_type_double:
        class_name = "Tjkjdfk0912jfkldHandleDob"
    elif insert_0905_type == global_0905insert_type_NSArray:
        class_name = "Tdjfk0912jfdiHandleAr"
    for insert_class_obj in use_insert_class_list:
        if insert_class_obj.class_name == class_name:
            return insert_class_obj
    return None
...
def get_insert_0905_class_obj(use_insert_class_list, insert_0905_type):
    class_name = ""
    if insert_0905_type == global_0905insert_type_longlong:
        class_name = "BTLdfjijfkldLo"
    elif insert_0905_type == global_0905insert_type_NSString:
        class_name = "BTLdfjkodsfjoSt"
    elif insert_0905_type == global_0905insert_type_double:
        class_name = "BTL3wjefjioDob"
    elif insert_0905_type == global_0905insert_type_NSArray:
        class_name = "BTLdfjiodfgfdjioAr"
    for insert_class_obj in use_insert_class_list:
        if insert_class_obj.class_name == class_name:
            return insert_class_obj
    return None
...
def reduction_0919_prototype_from_type(prototype_list, insert_0919_type):
    ...
    if insert_0919_type == global_0905insert_type_double:
        method_transfer_name = "IUEOuweKUoeijdIweokldldLoTixzvnpwe"       
    elif insert_0919_type == global_0905insert_type_longlong:
        method_transfer_name = "IUEOuweKUoeijdIweokldldLoFixnvmwpek"
    elif insert_0919_type == global_0905insert_type_NSString:
        method_transfer_name = "IUEOuweKUoeijdIweokldldLoNwohgnpwet"
    elif insert_0919_type == global_0905insert_type_NSArray:
        method_transfer_name = "IUEOuweKUoeijdIweokldldLoDiwoehgnpie"
...
def handle_0919_prototype_from_type(prototype_list, insert_0919_type):
    method_transfer_name = ""
    if insert_0919_type == global_0905insert_type_double:
        method_transfer_name = "IUEOuweKUoeijdIweokldldLoTYs5ugt"    
    elif insert_0919_type == global_0905insert_type_longlong:
        method_transfer_name = "IUEOuweKUoeijdIweokldldLoYsg4dfu"
    elif insert_0919_type == global_0905insert_type_NSString:
        method_transfer_name = "IUEOuweKUoeijdIweokldldLoKshdgh2jwe"
    elif insert_0919_type == global_0905insert_type_NSArray:
        method_transfer_name = "IUEOuweKUoeijdIweokldldLoKstd6hstd"
...
def generate_1026_ui():
    ...
    random_max_type = random.randrange(0, 13)
    ui_implamentation_str = ""
    if random_max_type == 0:
        ui_implamentation_str += "\tUISlider * " + ui_var_name + " = [[UISlider alloc] init];\n"
        ui_implamentation_str += "\t[" + ui_var_name + " setFrame:" + generate_0905_param_str("CGRect") + "];\n"
        random_num0 = random.randrange(0, 10)
        if random_num0 < 3:
            ui_implamentation_str += "\t[" + ui_var_name + " setMaskView:[UIView new]];\n"
        ...
        ui_implamentation_str += "\t" + ui_var_name + ".thumbTintColor = [UIColor clearColor];\n"
        random_num1 = random.randrange(0, 10)
        if random_num1 < 5:
            ui_implamentation_str += "\t" + ui_var_name + ".contentMode = "+generate_random_contenMode()+";\n"
        ui_implamentation_str += "\t" + ui_var_name + ".userInteractionEnabled = NO;\n"
        random_num2 = random.randrange(0, 10)
        if random_num2 < 3:
            ui_implamentation_str += "\t[" + ui_var_name + " setContinuous:" + generate_0905_param_str("BOOL") + "];\n"
        ui_implamentation_str += "\t" + ui_var_name + ".contentScaleFactor = "+generate_0905_param_str("double")+";\n"
        random_num3 = random.randrange(0, 10)
        if random_num3 < 5:        
            ui_implamentation_str += "\t[" + ui_var_name + " setClipsToBounds:" + generate_0905_param_str("BOOL") + "];\n"
        random_num4 = random.randrange(0, 10)
        if random_num4 < 3: 
            ui_implamentation_str += "\t[" + ui_var_name + " setLayoutMargins:"+generate_random_EdgeInsetsSet()+"];\n"
        ui_implamentation_str += "\t[" + ui_var_name + " setMinimumValue:0];\n"
        ui_implamentation_str += "\t[" + ui_var_name + " setMaximumValue:" + generate_0905_param_str("NSInteger") + "];\n"
        random_num5 = random.randrange(0, 10)
        if random_num4 < 5:         
            ui_implamentation_str += "\t[" + ui_var_name + " setThumbImage:[UIImage imageNamed:"+generate_0905_param_str("NSString *")+"] forState:UIControlStateNormal];\n"
        ui_implamentation_str += "\t[" + ui_var_name + " setAlpha:" + generate_0905_param_str("double") + "];\n"
        ...
        # ui_implamentation_str += "\t" + ui_var_name + ".contentScaleFactor = "+generate_0905_param_str("double")+";\n"        
        # ui_implamentation_str += "\t[" + ui_var_name + " setFrame:" + generate_0905_param_str("CGRect") + "];\n"
        # ui_implamentation_str += "\t[" + ui_var_name + " setDate:[NSDate dateWithTimeIntervalSinceNow:" + str(random.randrange(0, 1000)) + "] animated:YES];\n"
        # random_num1 = random.randrange(0, 10)
        # if random_num1 < 5:
        ...
        #UIImageView
        Long_value = generate_random_str(out_random_type_class)
        ui_implamentation_str += "\tUIImageView * " + ui_var_name + " = [[UIImageView alloc]initWithFrame:" + generate_0905_param_str("CGRect") + "];\n"
        ui_implamentation_str += "\t" + ui_var_name + ".backgroundColor = [UIColor clearColor];\n"
        ui_implamentation_str += "\tlong long "+Long_value+" = " + generate_0905_param_str("NSUInteger") + ";\n"
        ui_implamentation_str += "\t[" + ui_var_name + " setAnimationDuration:"+Long_value+"];\n"
        random_num1 = random.randrange(0, 10)
        if random_num1 < 3:
        ...
        random_num3 = random.randrange(0, 10)
        if random_num3 < 3:
            ui_implamentation_str += "\t" + ui_var_name + ".layer.borderColor = "+generate_rgb_color()+".CGColor;\n"
        random_num4 = random.randrange(0, 10)
        if random_num4 < 5:            
            ui_implamentation_str += "\t" + ui_var_name + ".clipsToBounds = "+generate_0905_param_str("BOOL")+";\n"
        ui_implamentation_str += "\t" + ui_var_name + ".userInteractionEnabled = NO;\n"

    elif random_max_type == 2:
        #UINavigationBar
        ui_implamentation_str += "\tUIProgressView *" + ui_var_name + " = [[UIProgressView alloc] initWithFrame:" + generate_0905_param_str("CGRect") + "];\n"
        ui_implamentation_str += "\t" + ui_var_name + ".backgroundColor = [UIColor clearColor];\n"
        ui_implamentation_str += "\t" + ui_var_name + ".tintColor = [UIColor clearColor];\n"
        ui_implamentation_str += "\t" + ui_var_name + ".trackTintColor = [UIColor clearColor];\n"
        ui_implamentation_str += "\t" + ui_var_name + ".userInteractionEnabled = NO;\n"

        random_num1 = random.randrange(0, 10)
        if random_num1 < 3:
            ui_implamentation_str += "\t[" + ui_var_name + " setProgressImage:[UIImage imageNamed:"+generate_0905_param_str("NSString *")+"]];\n"
        random_num2 = random.randrange(0, 10)
        if random_num2 < 5:        
            ui_implamentation_str += "\t" + ui_var_name + ".contentScaleFactor = "+generate_0905_param_str("double")+";\n"
        random_num3 = random.randrange(0, 10)
        ...
        random_num4 = random.randrange(0, 10)
        if random_num4 < 5:          
            ui_implamentation_str += "\t[" + ui_var_name + " setProgress:"+generate_0905_param_str("double")+"];\n"
        random_num5 = random.randrange(0, 10)
        if random_num5 < 3:
            ui_implamentation_str += "\t[" + ui_var_name + " setAlpha:"+generate_0905_param_str("double")+"];\n"
        random_num6 = random.randrange(0, 10)
        ...
        random_num7 = random.randrange(0, 10)
        if random_num7 < 3:
            ui_implamentation_str += "\t[" + ui_var_name + " setOpaque:"+generate_0905_param_str("BOOL")+"];\n"                 
        ...
        random_num2 = random.randrange(0, 10)
        if random_num2 < 5:
            double_value = generate_random_str(out_random_type_class)
        ...
        random_num3 = random.randrange(0, 10)
        if random_num3 < 3:            
            ui_implamentation_str += "\t" + ui_var_name + ".alpha = "+generate_0905_param_str("double")+";\n"
        random_num4 = random.randrange(0, 10)
        if random_num4 < 5:
            ui_implamentation_str += "\t" + ui_var_name + ".highlighted = " + generate_0905_param_str("BOOL") + ";\n"
    elif random_max_type == 4:
        #UIInputView
        ui_implamentation_str += "\tUIInputView *" + ui_var_name + " = [[UIInputView alloc]initWithFrame:"+generate_0905_param_str("CGRect")+" inputViewStyle:UIInputViewStyleDefault];\n"
        ui_implamentation_str += "\t" + ui_var_name + ".userInteractionEnabled = NO;\n"
        random_num0 = random.randrange(0, 10)
        if random_num0 < 3:        
            ui_implamentation_str += "\t" + ui_var_name + ".contentScaleFactor = "+generate_0905_param_str("double")+";\n"
        ...
        if random_num1 < 5:
            ui_implamentation_str += "\t[" + ui_var_name + " setFrame:" + generate_0905_param_str("CGRect") + "];\n"
            ui_implamentation_str += "\t[" + ui_var_name + " setTintColor:"+generate_rgb_color()+"];\n"
            ui_implamentation_str += "\t[" + ui_var_name + " setBackgroundColor:[UIColor clearColor]];\n"
        ui_implamentation_str += "\t[" + ui_var_name + " setAllowsSelfSizing:" + generate_0905_param_str("BOOL") + "];\n"
    elif random_max_type == 5:
        #UIScrollView
        view_var = generate_random_str(out_random_type_class)
        ui_implamentation_str += "\tUIScrollView * " + ui_var_name + " = [[UIScrollView alloc] initWithFrame:" + generate_0905_param_str("CGRect") + "];\n"
        ui_implamentation_str += "\tUIImageView * "+view_var+" = [[UIImageView alloc] initWithImage:[UIImage imageNamed:" + generate_0905_param_str("NSString *") + "]];\n"
        ui_implamentation_str += "\t["+view_var+" setBackgroundColor:[UIColor clearColor]];\n"
        random_num0 = random.randrange(0, 10)
        if random_num0 < 3:
            # Long_value3 = generate_random_str(out_random_type_class)
            ui_implamentation_str += "\t["+view_var+" setAnimationDuration:1];\n"
            ui_implamentation_str += "\t["+view_var+" setHighlighted:" + generate_0905_param_str("BOOL") + "];\n"
            # ui_implamentation_str += "\tlong long "+Long_value3+" = " + generate_0905_param_str("NSInteger") + ";\n"
            ui_implamentation_str += "\t" + ui_var_name + ".contentSize = "+generate_random_contentSizeMake()+";\n"
        ...
        ui_implamentation_str += "\t" + ui_var_name + ".backgroundColor = [UIColor clearColor];\n"
        ui_implamentation_str += "\t" + ui_var_name + ".font = "+generate_0905_param_str("UIFont *")+";\n"
        random_num1 = random.randrange(0, 10)
        if random_num1 < 3:
            ui_implamentation_str += "\t" + ui_var_name + ".textAlignment = "+generate_random_textAlignment()+";\n"
        ...
        random_num4 = random.randrange(0, 10)
        if random_num4 < 3:
            ui_implamentation_str += "\t" + ui_var_name + ".returnKeyType = " + generate_random_return_type() + ";\n"
        random_num5 = random.randrange(0, 10)
        if random_num5 < 5:
            ui_implamentation_str += "\t" + ui_var_name + ".keyboardType = " + generate_random_Keyboard_type() + ";\n"
        random_num6 = random.randrange(0, 10)
        if random_num6 < 3:
            ui_implamentation_str += "\t" + ui_var_name + ".keyboardAppearance = " + generate_random_Keyboard_color() + ";\n"
        random_num7 = random.randrange(0, 10)
        if random_num7 < 3:
            ui_implamentation_str += "\t" + ui_var_name + ".keyboardDismissMode = UIScrollViewKeyboardDismissModeNone;\n"
        ui_implamentation_str += "\t" + ui_var_name + ".clipsToBounds = "+generate_0905_param_str("BOOL")+";\n"
        random_num8 = random.randrange(0, 10)
        if random_num8 < 5:
            ui_implamentation_str += "\t" + ui_var_name + ".contentInset = "+generate_random_EdgeInsetsSet()+";\n"
        ui_implamentation_str += "\t" + ui_var_name + ".contentMode = "+generate_random_contenMode()+";\n"
        random_num9 = random.randrange(0, 10)
        if random_num9 < 3:
            ui_implamentation_str += "\t" + ui_var_name + ".contentSize = "+generate_random_contentSizeMake()+";\n"
        ui_implamentation_str += "\t" + ui_var_name + ".alpha = "+generate_0905_param_str("double")+";\n"
        random_num10 = random.randrange(0, 10)
        if random_num10 < 5:      
            ui_implamentation_str += "\t" + ui_var_name + ".contentOffset = "+generate_0905_param_str("CGPoint")+";\n"

        random_text_str = generate_random_str(out_random_type_class)
        random_attribute_str = generate_random_str(out_random_type_class)

        random_num11 = random.randrange(0, 10)
        if random_num11 < 3: 
            ui_implamentation_str += "\tNSString *" + random_text_str + " = " + generate_0905_param_str("NSString *") + ";\n"

            ui_implamentation_str += "\tNSMutableAttributedString *" + random_attribute_str + " = [[NSMutableAttributedString alloc] initWithString:"+random_text_str+"];\n"
            random_num12 = random.randrange(0, 10)%3
            if random_num12 == 0:
                ui_implamentation_str += "\t[" + random_attribute_str + " addAttribute:"+generate_random_Attributed_color()+" value:"+generate_rgb_color()+" range:NSMakeRange(0, "+random_attribute_str+".length)];\n"                
                
            elif random_num12 == 1:
                ui_implamentation_str += "\t[" + random_attribute_str + " addAttribute:"+generate_random_Attributed_number()+" value:@"+generate_0905_param_str("NSInteger")+" range:NSMakeRange(0, "+random_attribute_str+".length)];\n"                

            elif random_num12 == 2:
                ui_implamentation_str += "\t[" + random_attribute_str + " addAttribute:"+generate_random_Attributed_double()+" value:@"+generate_0905_param_str("double")+" range:NSMakeRange(0, "+random_attribute_str+".length)];\n"                

            ui_implamentation_str += "\t" + ui_var_name + ".text = " + random_text_str + ";\n"

    elif random_max_type == 7:
        # UITextField
        ui_implamentation_str += "\tUITextField *" + ui_var_name + " = [[UITextField alloc] init];\n"
        ui_implamentation_str += "\t" + ui_var_name + ".frame = "+generate_0905_param_str("CGRect")+";\n"
        ui_implamentation_str += "\t" + ui_var_name + ".backgroundColor = [UIColor clearColor];\n"
        ui_implamentation_str += "\t" + ui_var_name + ".userInteractionEnabled = NO;\n"

        random_num1 = random.randrange(0,10)
        if random_num1 < 3:
            ui_implamentation_str += "\t" + ui_var_name + ".borderStyle = UITextBorderStyleNone;\n"
        random_num2 = random.randrange(0,10)
        if random_num2 < 5:            
            ui_implamentation_str += "\t" + ui_var_name + ".placeholder = @\"\";\n"
        random_num3 = random.randrange(0,10)
        if random_num3 < 3:             
            ui_implamentation_str += "\t" + ui_var_name + ".clearsOnBeginEditing = "+generate_0905_param_str("BOOL")+";\n"
        random_num4 = random.randrange(0,10)
        if random_num4 < 5:             
            ui_implamentation_str += "\t" + ui_var_name + ".keyboardAppearance = "+generate_random_Keyboard_color()+";\n"
        random_num5 = random.randrange(0,10)
        if random_num5 < 3:             
            ui_implamentation_str += "\t" + ui_var_name + ".secureTextEntry = "+generate_0905_param_str("BOOL")+";\n"
        random_num6 = random.randrange(0,10)
        if random_num6 < 5:             
            ui_implamentation_str += "\t" + ui_var_name + ".returnKeyType = "+generate_random_return_type()+";\n"
        random_num7 = random.randrange(0,10)
        if random_num7 < 3:             
            ui_implamentation_str += "\t" + ui_var_name + ".clearButtonMode = "+generate_clear_leftview_mode()+";\n"
        random_num8 = random.randrange(0,10)
        if random_num8 < 5: 
            ui_implamentation_str += "\t" + ui_var_name + ".leftViewMode = "+generate_clear_leftview_mode()+";\n"
        random_num9 = random.randrange(0,10)
        if random_num9 < 3: 
            ui_implamentation_str += "\t[" + ui_var_name + " becomeFirstResponder];\n"                
        random_num10 = random.randrange(0,10)
        if random_num10 < 5: 
            random_views = generate_random_str(out_random_type_class)
            ui_implamentation_str += "\tUIView *" + random_views + " = [[UIView alloc] initWithFrame:" + generate_0905_param_str("CGRect") + "];\n"
            ui_implamentation_str += "\t" + random_views + ".backgroundColor = [UIColor clearColor];\n"
            random_num11 = random.randrange(0,10)%3
            if random_num11 == 0: 
                ui_implamentation_str += "\t" + ui_var_name + ".inputView = "+random_views+";\n"
            elif random_num11 == 1:
                ui_implamentation_str += "\t" + ui_var_name + ".inputAccessoryView = "+random_views+";\n"
            elif random_num11 == 2:
                ui_implamentation_str += "\t" + ui_var_name + ".leftView = "+random_views+";\n"

        random_num12 = random.randrange(0,10)
        if random_num12 < 5:                
            ui_implamentation_str += "\t" + ui_var_name + ".layer.borderColor = "+generate_rgb_color()+".CGColor;\n"
            ui_implamentation_str += "\t" + ui_var_name + ".layer.borderWidth = "+generate_0905_param_str("double")+";\n"
        random_num13 = random.randrange(0,10)
        if random_num13 < 3:           
            ui_implamentation_str += "\t" + ui_var_name + ".layer.shadowColor = "+generate_rgb_color()+".CGColor;\n"
            ui_implamentation_str += "\t" + ui_var_name + ".layer.shadowOffset = "+generate_random_offSetSizeMake()+";\n"
            ui_implamentation_str += "\t" + ui_var_name + ".layer.cornerRadius = "+generate_0905_param_str("double")+";\n"
            ui_implamentation_str += "\t" + ui_var_name + ".layer.shadowRadius = "+generate_0905_param_str("double")+";\n"

    elif random_max_type == 8:
        #UIView
        ui_implamentation_str += "\tUIView *" + ui_var_name + " = [[UIView alloc] initWithFrame:" + generate_0905_param_str("CGRect") + "];\n"
        random_num0 = random.randrange(0, 10)
        if random_num0 < 5:
            random_blurEffect = generate_random_str(out_random_type_class)
            random_effectView = generate_random_str(out_random_type_class)
            ui_implamentation_str += "\tUIBlurEffect *" + random_blurEffect + " = [UIBlurEffect effectWithStyle:"+generate_blurEffect_style()+"];\n"
            ui_implamentation_str += "\tUIVisualEffectView *" + random_effectView + " = [[UIVisualEffectView alloc] initWithEffect:"+random_blurEffect+"];\n"
            ui_implamentation_str += "\t" + random_effectView + ".frame = "+ui_var_name+".bounds;\n"
            # ui_implamentation_str += "\t" + random_effectView + ".alpha = "+generate_0905_param_str("double")+";\n"
            ui_implamentation_str += "\t[" + ui_var_name + " addSubview:" + random_effectView + "];\n"
        ui_implamentation_str += "\t" + ui_var_name + ".userInteractionEnabled = NO;\n"
        ui_implamentation_str += "\t" + ui_var_name + ".contentScaleFactor = "+generate_0905_param_str("double")+";\n"        
        ui_implamentation_str += "\t" + ui_var_name + ".backgroundColor = [UIColor clearColor];\n"
        random_num1 = random.randrange(0, 10)
        ...
        if random_num2 < 5:        
            ui_implamentation_str += "\t" + ui_var_name + ".contentMode = "+generate_random_contenMode()+";\n"
        ui_implamentation_str += "\t" + ui_var_name + ".transform = CGAffineTransformRotate("+ui_var_name+".transform, "+generate_transofm_Rotate()+");\n"
        random_num3 = random.randrange(0, 10)
        if random_num3 < 3:         
            ui_implamentation_str += "\t[" + ui_var_name + " setOpaque:" + generate_0905_param_str("BOOL") + "];\n"                
        ui_implamentation_str += "\t[" + ui_var_name + " setLayoutMargins:"+generate_random_EdgeInsetsSet()+"];\n"
        random_num4 = random.randrange(0, 10)
        if random_num4 < 5:                          
            ui_implamentation_str += "\t[" + ui_var_name + " setMultipleTouchEnabled:" + generate_0905_param_str("BOOL") + "];\n"                
        ui_implamentation_str += "\t" + ui_var_name + ".alpha = "+generate_0905_param_str("double")+";\n"

    elif random_max_type == 9:
        ui_implamentation_str += "\tUISegmentedControl *" + ui_var_name + " = [[UISegmentedControl alloc] init];\n"
        ui_implamentation_str += "\t" + ui_var_name + ".frame = "+generate_0905_param_str("CGRect")+";\n"        
  
        random_num1 = random.randrange(0, 10)               
        if random_num1 < 3:
            ui_implamentation_str += "\t" + ui_var_name + ".contentHorizontalAlignment = "+generate_random_ControlContentHorizontalAlignment()+";\n"
        ui_implamentation_str += "\t" + ui_var_name + ".contentScaleFactor = "+generate_0905_param_str("double")+";\n"                
        random_num2 = random.randrange(0, 10)
        if random_num2 < 5:
            ui_implamentation_str += "\t[" + ui_var_name + " setMomentary:" + generate_0905_param_str("BOOL") + "];\n"
        ui_implamentation_str += "\t" + ui_var_name + ".userInteractionEnabled = NO;\n"
        
        random_num3 = random.randrange(0, 10)
        if random_num3 < 3:            
            ui_implamentation_str += "\t[" + ui_var_name + " setSelectedSegmentIndex:" + generate_0905_param_str("double") + "];\n"  
        ui_implamentation_str += "\t" + ui_var_name + ".alpha = " + generate_0905_param_str("double") + ";\n"
        
        random_num4 = random.randrange(0, 10)
        if random_num4 < 5:                          
            ui_implamentation_str += "\t[" + ui_var_name + " setTag:" + generate_0905_param_str("NSInteger") + "];\n"    
        ui_implamentation_str += "\t" + ui_var_name + ".backgroundColor = [UIColor clearColor];\n"
        ui_implamentation_str += "\t" + ui_var_name + ".tintColor = [UIColor clearColor];\n"                         
    elif random_max_type == 10:
        ui_implamentation_str += "\tUISwitch *" + ui_var_name + " = [[UISwitch alloc] initWithFrame:"+generate_0905_param_str("CGRect")+"];\n"       
        ui_implamentation_str += "\t" + ui_var_name + ".backgroundColor = [UIColor clearColor];\n"
        ui_implamentation_str += "\t" + ui_var_name + ".tintColor = [UIColor clearColor];\n" 
        ui_implamentation_str += "\t" + ui_var_name + ".thumbTintColor = [UIColor clearColor];\n" 
        random_num0 = random.randrange(0, 10)
        if random_num0 < 5:          
            ui_implamentation_str += "\t[" + ui_var_name + " setOn:" + generate_0905_param_str("BOOL") + "];\n"
        ui_implamentation_str += "\t[" + ui_var_name + " setLayoutMargins:"+generate_random_EdgeInsetsSet()+"];\n"
        random_num1 = random.randrange(0, 10)
        if random_num1 < 3:          
            ui_implamentation_str += "\t[" + ui_var_name + " setClipsToBounds:" + generate_0905_param_str("BOOL") + "];\n"
        ...
        if random_num2 < 5:         
            ui_implamentation_str += "\t[" + ui_var_name + " setTag:" + generate_0905_param_str("NSInteger") + "];\n"
        ui_implamentation_str += "\t[" + ui_var_name + " setPreservesSuperviewLayoutMargins:" + generate_0905_param_str("BOOL") + "];\n"
        random_num3 = random.randrange(0, 10)
        if random_num3 < 3:         
            ui_implamentation_str += "\t[" + ui_var_name + " setTransform:CGAffineTransformScale("+ui_var_name+".transform, 0."+str(random.randrange(50,85))+", 0."+str(random.randrange(50,85))+")];\n"
        ui_implamentation_str += "\t" + ui_var_name + ".userInteractionEnabled = NO;\n"
        ui_implamentation_str += "\t" + ui_var_name + ".contentScaleFactor = "+generate_0905_param_str("double")+";\n"    
    elif random_max_type == 11:
        ui_implamentation_str += "\tUIButton *" + ui_var_name + " = [UIButton buttonWithType:"+generate_button_style()+"];\n"
        ui_implamentation_str += "\t" + ui_var_name + ".frame = "+generate_0905_param_str("CGRect")+";\n"                
        ui_implamentation_str += "\t" + ui_var_name + ".backgroundColor = [UIColor clearColor];\n"
        ui_implamentation_str += "\t" + ui_var_name + ".tintColor = [UIColor clearColor];\n" 
        ui_implamentation_str += "\t" + ui_var_name + ".titleLabel.textColor = [UIColor clearColor];\n" 
        random_num0 = random.randrange(0, 10)
        if random_num0 < 5: 
            ui_implamentation_str += "\t[" + ui_var_name + " setTag:" + generate_0905_param_str("NSInteger") + "];\n"
        ui_implamentation_str += "\t[" + ui_var_name + " setAlpha:" + generate_0905_param_str("double") + "];\n"
        random_num1 = random.randrange(0, 10)
        if random_num1 < 3:
            ui_implamentation_str += "\t[" + ui_var_name + " setTitle:"+generate_0905_param_str("NSString *")+" forState:UIControlStateNormal];\n"
            ui_implamentation_str += "\t[" + ui_var_name + " setTitle:"+generate_0905_param_str("NSString *")+" forState:UIControlStateHighlighted];\n"
        ui_implamentation_str += "\t" + ui_var_name + ".userInteractionEnabled = NO;\n"
        random_num2 = random.randrange(0, 10)
        if random_num2 < 5:            
            ui_implamentation_str += "\t[" + ui_var_name + " setTitleEdgeInsets:"+generate_random_EdgeInsetsSet()+"];\n"
        ui_implamentation_str += "\t" + ui_var_name + ".contentScaleFactor = "+generate_0905_param_str("double")+";\n"                
        random_num3 = random.randrange(0, 10)
        if random_num3 < 3:         
            ui_implamentation_str += "\t[" + ui_var_name + " setImageEdgeInsets:"+generate_random_EdgeInsetsSet()+"];\n"
        random_num4 = random.randrange(0, 10)
        if random_num4 < 5:             
            ui_implamentation_str += "\t[" + ui_var_name + " setContentEdgeInsets:"+generate_random_EdgeInsetsSet()+"];\n"
        ui_implamentation_str += "\t[" + ui_var_name + " setShowsTouchWhenHighlighted:"+generate_0905_param_str("BOOL")+"];\n"
        random_num5 = random.randrange(0, 10)
        if random_num5 < 3:        
            ui_implamentation_str += "\t[" + ui_var_name + " setClipsToBounds:"+generate_0905_param_str("BOOL")+"];\n"
        random_num6 = random.randrange(0, 10)
        if random_num6 < 5:
            random_attribute_str = generate_random_str(out_random_type_class)
            ui_implamentation_str += "\tNSAttributedString *" + random_attribute_str + " = [[NSAttributedString alloc] initWithString:"+generate_0905_param_str("NSString *")+" attributes:@{NSForegroundColorAttributeName:"+generate_rgb_color()+",NSAttachmentAttributeName:"+generate_0905_param_str("UIFont *")+"}];\n"
            ui_implamentation_str += "\t[" + ui_var_name + " setAttributedTitle:"+random_attribute_str+" forState:UIControlStateHighlighted];\n"
        random_num7 = random.randrange(0, 10)
        if random_num7 < 3:
            ui_implamentation_str += "\t" + ui_var_name + ".layer.cornerRadius = " + generate_0905_param_str("double") + ";\n"
            ui_implamentation_str += "\t" + ui_var_name + ".layer.borderColor = "+generate_rgb_color()+".CGColor;\n"
    ...
    elif random_max_type == 12:
        ui_implamentation_str += "\tUILabel *" + ui_var_name + " = [[UILabel alloc] initWithFrame:"+generate_0905_param_str("CGRect")+"];\n"
        ui_implamentation_str += "\t" + ui_var_name + ".userInteractionEnabled = NO;\n"
        ui_implamentation_str += "\t" + ui_var_name + ".contentScaleFactor = "+generate_0905_param_str("double")+";\n"        
        ui_implamentation_str += "\t" + ui_var_name + ".backgroundColor = [UIColor clearColor];\n"
        ui_implamentation_str += "\t" + ui_var_name + ".tintColor = [UIColor clearColor];\n" 
        ...
        if random_num0 < 3:
            ui_implamentation_str += "\t" + ui_var_name + ".transform = CGAffineTransformRotate("+ui_var_name+".transform, "+generate_transofm_Rotate()+");\n"        
        random_mut_attribute_str = generate_random_str(out_random_type_class)
        ui_implamentation_str += "\tNSMutableAttributedString *" + random_mut_attribute_str + " = [[NSMutableAttributedString alloc] initWithString:"+generate_0905_param_str("NSString *")+"];\n"
        random_num1 = random.randrange(0, 10)
        if random_num1 < 5:         
            ui_implamentation_str += "\t[" + random_mut_attribute_str + " addAttribute:"+generate_random_Attributed_color()+" value:"+generate_rgb_color()+" range:NSMakeRange(0, "+random_mut_attribute_str+".length)];\n"
        random_num2 = random.randrange(0, 10)
        if random_num2 < 3:         
            ui_implamentation_str += "\t[" + random_mut_attribute_str + " addAttribute:"+generate_random_Attributed_double()+" value:@0."+str(random.randrange(1,10))+" range:NSMakeRange(0, "+random_mut_attribute_str+".length)];\n"
        random_num3 = random.randrange(0, 10)
        if random_num3 < 3:         
            ui_implamentation_str += "\t[" + random_mut_attribute_str + " addAttribute:NSAttachmentAttributeName value:"+generate_0905_param_str("UIFont *")+" range:NSMakeRange(0, "+random_mut_attribute_str+".length)];\n"
        random_num4 = random.randrange(0, 10)
        if random_num4 < 5:        
            ui_implamentation_str += "\t[" + random_mut_attribute_str + " addAttribute:"+generate_random_Attributed_number()+" value:@"+str(random.randrange(2,10))+" range:NSMakeRange(0, "+random_mut_attribute_str+".length)];\n"
        
        random_paragraph_style = generate_random_str(out_random_type_class)
        ui_implamentation_str += "\tNSMutableParagraphStyle *" + random_paragraph_style + " = [[NSMutableParagraphStyle alloc] init];\n"        
        ui_implamentation_str += "\t" + random_paragraph_style + ".firstLineHeadIndent = "+str(random.randrange(10,20))+";\n"
        random_num5 = random.randrange(0, 10)
        if random_num5 < 3:         
            ui_implamentation_str += "\t" + random_paragraph_style + ".lineBreakMode = "+generate_random_lineBreakMode()+";\n"
        ui_implamentation_str += "\t" + random_paragraph_style + ".lineSpacing = "+str(random.randrange(2,10))+";\n"
        random_num6 = random.randrange(0, 10)
        if random_num6 < 3:   
            ui_implamentation_str += "\t" + random_paragraph_style + ".lineHeightMultiple = "+str(random.randrange(2,10))+";\n"
        ui_implamentation_str += "\t[" + random_mut_attribute_str + " addAttribute:NSParagraphStyleAttributeName value:"+random_paragraph_style+" range:NSMakeRange(0, "+random_mut_attribute_str+".length)];\n"
        
        random_num7 = random.randrange(0, 10)
        if random_num7 < 3:
            random_shadow = generate_random_str(out_random_type_class)
            ui_implamentation_str += "\tNSShadow *" + random_shadow + " = [[NSShadow alloc] init];\n"        
            ui_implamentation_str += "\t" + random_shadow + ".shadowColor = "+generate_rgb_color()+";\n"
            ui_implamentation_str += "\t" + random_shadow + ".shadowOffset = "+generate_random_offSetSizeMake()+";\n"
            ui_implamentation_str += "\t[" + random_mut_attribute_str + " addAttribute:NSShadowAttributeName value:"+random_shadow+" range:NSMakeRange(0, "+random_mut_attribute_str+".length)];\n"
            
        ui_implamentation_str += "\t[" + ui_var_name + " setAttributedText:" + random_mut_attribute_str + "];\n"
...
def main():
    ...
    # 0905版本插入点自动添加插入点标记--替换还原标记1
    # handle_add_insert_symble_0905()

    # 1108版本对未添加标记的函数添加标记-替换还原标记2
