...
class_prefix_str = "Euis"

nosearch_directorys = ["Pods", "AvoidCrash", "WMPageController"]
...
def  handle_transfer_defend_Garden5():
    ...
    random_num1 = random.randrange(0,100)%5
    if random_num1 == 0:
        # //手机别名： 用户定义的名称
    ...
    random_comp_str = "@\"" +generate_words_str("random_str")+ "\""
    out_str += "\tlong "+random_long+" = ["+random_str+" compare:"+random_comp_str+"];\n"
    out_str += "\t"+random_str+" = ["+random_long+" stringValue];\n"
    out_str += "\tNSMutableString *"+random_str+" = ["+random_str+" mutableCopy];\n"
    out_str += "\t"
    random_num2 = random.randrange(0,10)
    if random_num2 < 3:
        out_str += "\t["+random_str+" insertString:@\"" +generate_words_str("random_str")+ "\" atIndex:arc4random()%"+random_str+".length];\n"
    else:
        out_str += "\t["+random_str+" setString:"+random_comp_str+"];\n"
    random_num3 = random.randrange(0,10)
    if random_num3 < 5:
        out_str += "\t"+random_from_str+" = ["+random_str+" substringFromIndex:arc4random()%"+random_str+".length];\n"
        out_str += "\t"+random_from_str+" = ["+random_str+" substringFromIndex:arc4random()%"+random_str+".length];\n"
    else:
        out_str += "\t"+random_arr+" = ["+random_str+" componentsSeparatedByString:"+random_comp_str+"];\n"
        out_str += "\t["+random_str+" stringByReplacingOccurrencesOfString:"+random_arr+"[0] withString:"+random_arr+"[1]];\n"
    random_num4 = random.randrange(0,10)
    if random_num4 < 5:
        out_str += "\tNSRange "+random_range+" = ["+random_str+" rangeOfString:"+random_comp_str+"];\n"
        out_str += "\t["+random_str+" replaceCharactersInRange:range4 withString:@\"" +generate_words_str("random_str")+ "\"];\n"
    else:
        out_str += "\tNSRange "+random_range+" = ["+random_str+" rangeOfString:"+random_comp_str+"];\n"
        out_str += "\t"+random_str+" = [["+random_str+" substringWithRange:range1] mutableCopy];\n"
    ...
    if random_num0 < 5:
        ...
        out_str += "\tconst char *"+random_char+" = NULL;\n"        
        out_str += "\t"+random_index+" = arc4random()%"+random_str+".length;\n"
        out_str += "\t"+random_char+" = ["+random_str+" characterAtIndex:"+random_index+"];\n"
        out_str += "\t"+random_str+" = [NSString stringWithCString:"+random_char+"encoding:"+NSUTF8StringEncoding+"];\n"

    return out_str
# 处理0723DefendGarden申诉调用
def handle_transfer_defend_Garden(param_str):
    out_str = ""
    var1 = generate_random_str(out_random_type_method_name)
    out_str += "\tNSString *" + var1 + " = [" + param_str + " substringFromIndex:" + str(random.randrange(0, 5)) + "];\n"
    var2 = generate_random_str(out_random_type_method_name)
    out_str += "\tNSMutableString *" + var2 + " = [NSMutableString stringWithString:" + var1 + "];\n"
    for i in range(random.randrange(1, 5)):
        var3 = generate_random_str(out_random_type_method_name)
        out_str += "\tNSString *" + var3 + " = [" + param_str + " substringToIndex:" + str(random.randrange(5, 15)) + "];\n"
        out_str += "\t[" + var2 + " appendString:" + var3 + "];\n"
    return out_str
        ...
        if len(file_context) > 0:
            ...
            for line in file_context:
                if "//////insert//" in line:
                    ...
                    # need_replace_str += generate_handle_string_data(random_var)
                    need_replace_str += handle_transfer_defend_Garden(random_var)
...
def generate_0905_param_str(param_type):
    ...
    if "NSString *" in param_type:
        ...
        out_str = "CGRectMake(" + str(random.randrange(0, 100)) + ", " + str(random.randrange(0, 100)) + ", " + str(random.randrange(0, 100)) + ", " + str(random.randrange(0, 100)) + ")"
    elif "int" in param_type or "NSInteger" in param_type:
        out_str = str(random.randrange(0, 100))
        ...
        out_str = str(random.randrange(0, 100)) + "." + str(random.randrange(0, 100))
...
def reduction_0919_prototype_from_type(prototype_list, insert_0919_type):
    ...
    if insert_0919_type == global_0905insert_type_NSString:
        method_transfer_name = "jfdkllkfgldlfsldjlfi"
    elif insert_0919_type == global_0905insert_type_NSArray:
        method_transfer_name = "jkdfjkgjakdhjkdhfjhiw"
    elif insert_0919_type == global_0905insert_type_longlong:
        method_transfer_name = "fhdksgkfdsklfdlsfdsf"
    elif insert_0919_type == global_0905insert_type_double:
        method_transfer_name = "fdhkdjklnisoajnfks"
...
def handle_0919_prototype_from_type(prototype_list, insert_0919_type):
    ...
    if insert_0919_type == global_0905insert_type_double:
        method_transfer_name = "TisddiFhgeourEJbd3sojgn"
    elif insert_0919_type == global_0905insert_type_longlong:
        method_transfer_name = "TisddiFhgeourRdsbh9gierh"
    elif insert_0919_type == global_0905insert_type_NSString:
        method_transfer_name = "TisddiFhgeourLysd7fudhk"
    elif insert_0919_type == global_0905insert_type_NSArray:
        method_transfer_name = "TisddiFhgeourEtjdfb5hid"        
    for prototype_obj in prototype_list:
        if method_transfer_name in prototype_obj.transfer_name:
            return prototype_obj
    return None
